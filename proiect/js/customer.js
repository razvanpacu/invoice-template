...
function getCustomerDetails() { // functia care populeaza DOM
    var suppliersList = [];
    var customersList = [];

    fetch(apiLink + "customers")

        .then(function (response) {
            return response.json();
        })
        .then(function (customers) { // cautam furnizorul pentru factura returnata de searchInvoices
            customersList = customers;

        })
        .then(function () {
            populateDOM(customersList); //afisam in DOM factura
        })
        .then(function () { // dupa publicare - asteptam eventuri
            getClickDOM(customersList);
        })
}
...
function deleteRow(id) {
    ...
    var name = x.cells[1].innerText; //preluam numele din coloana a doua a tabelului
    var cui = x.cells[2].innerText; //preluam CUI din coloana a doua a tabelului
    fetch("http://beta.apexcode.ro/api/customers/" + elementId, { //!!!!! pentru customers/products trebuie schimbat api link
...
function publishNew() {
    ...
    if (cui.length > 10 || cui.length < 6) { // verificam daca CUI este lungime buna 
        ...
        fetch("http://beta.apexcode.ro/api/customers", { //!!!ATENTIE LA CALE
...
function publishEdited(id) {
    ...
    if (newCui.length > 10 || newCui.length < 6) { // verificam daca CUI este lungime buna 
        ...
        fetch("http://beta.apexcode.ro/api/customers/" + elementId, { //!!!ATENTIE LA CALE
